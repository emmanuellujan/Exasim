/*
================================================================================
GMRESMPIR Solver Implementation
================================================================================

This file implements the Generalized Minimal Residual (GMRESMPIR) iterative solver 
...
1. void CGSMPIR(cublasHandle_t handle, dstype *V, dstype *H, dstype *temp, Int N, Int m, Int backend)
...
2. void ApplyPolyMPIR(dstype *w, CDiscretization &disc, CPreconditioner& prec,
...
3. void UpdateSolutionMPIR(cublasHandle_t handle, dstype *x, dstype *y, dstype *H, dstype *s, dstype *V,
                             Int i, Int N, Int n, Int backend)
    - Updates the solution vector using the computed Krylov basis and coefficients.

4. Int GMRESMPIR(sysstruct &sys, CDiscretization &disc, CPreconditioner& prec, Int backend)
    - Main GMRESMPIR solver routine.
...
5. void ApplyPolyMPIR(dstype *w, CDiscretization &disc, CPreconditioner& prec,
                        sysstruct &sys, dstype *q, dstype *p, int N, Int spatialScheme, int backend)
    - Overloaded version of ApplyPolyMPIR with spatialScheme parameter for flexibility.

6. Int GMRESMPIR(sysstruct &sys, CDiscretization &disc, CPreconditioner& prec, Int N, Int spatialScheme, Int backend)
    - Overloaded GMRESMPIR routine supporting additional spatial scheme parameter.
...
- Restart: Limits the number of Krylov vectors to control memory usage and 
  improve robustness.
- Orthogonalization: Supports both Classical (CGSMPIR) and Modified Gram-Schmidt (MGS).
...
Call GMRESMPIR with appropriate system, discretization, and preconditioner objects.
...
void CGSMPIR(cublasHandle_t handle, dstype *V, dstype *H, dstype *temp, Int N, Int m, Int backend)
{
...
void ApplyPolyMPIR(dstype *w, CDiscretization &disc, CPreconditioner& prec,
        sysstruct &sys, dstype *q, dstype *p, int N, int backend)
{
...
void UpdateSolutionMPIR(cublasHandle_t handle, dstype *x, dstype *y, dstype *H, dstype *s, dstype *V,
        Int i, Int N, Int n, Int backend)
{
...
Int GMRESMPIR(sysstruct &sys, CDiscretization &disc, CPreconditioner& prec, Int backend)
{
    ...
    if (disc.common.ppdegree>1) {
        ...
        ApplyPolyMPIR(sys.r, disc, prec, sys, sys.q, sys.p, N, backend);    
    }
    ...
    while (j < maxit) {
        ...
        for (i = 0; i < nrest && j < maxit; i++, j++) {
            ...
            if (disc.common.ppdegree>1) {
                ...
                ApplyPolyMPIR(&sys.v[m*N], disc, prec, sys, sys.q, sys.p, N, backend);
            }
            else if (disc.common.RBcurrentdim>=0) {
                prec.ApplyPreconditioner(&sys.v[m*N], sys, disc, backend);   
                //ApplyComponentNorm(disc, sys.normcu, &sys.v[m*N], disc.res.Ru, ncu, ncu, npe, ne, backend);
            ...
            if (orthogMethod == 0)
                ...
                CGSMPIR(disc.common.cublasHandle, sys.v, &H[n1*i], y, N, m, backend);
            END_TIMING_DISC(2);    
            ...
            if (disc.common.linearSolverRelError < tol) {                
                UpdateSolutionMPIR(disc.common.cublasHandle, sys.x, y, H, s, sys.v, i, N, n1, backend);
                return j;
            }                        
            END_TIMING_DISC(3);    
        }        
        ...
        UpdateSolutionMPIR(disc.common.cublasHandle, sys.x, y, H, s, sys.v, nrest-1, N, n1, backend);
    ...
    if (disc.common.linearSolverRelError > tol) {
        if (disc.common.mpiRank==0) {
            printf("Warning: GMRESMPIR(%d) does not converge to the tolerance %g within % d iterations\n",nrest,tol,maxit);
...
Int GMRESMPIR(sysstruct &sys, CDiscretization &disc, CPreconditioner& prec, Int N, Int spatialScheme, Int backend)
{
    ...
    if (disc.common.ppdegree>1) {
        ...
        ApplyPolyMPIR(sys.r, disc, prec, sys, sys.q, sys.p, N, spatialScheme, backend);    
    }
    else if (disc.common.RBcurrentdim>=0) {
        prec.ApplyPreconditioner(sys.r, sys, disc, spatialScheme, backend);
    }
    ...
    hipDeviceSynchronize();
            ...
            if (disc.common.ppdegree>1) {
                ...
                ApplyPolyMPIR(&sys.v[m*N], disc, prec, sys, sys.q, sys.p, N, spatialScheme, backend);
            }
            else if (disc.common.RBcurrentdim>=0) {
                prec.ApplyPreconditioner(&sys.v[m*N], sys, disc, spatialScheme, backend);   
            }
    ...
    hipDeviceSynchronize();
        ...
        // update solution: x = x + v*s
        UpdateSolutionMPIR(disc.common.cublasHandle, sys.x, y, H, s, sys.v, i-1, N, n1, backend);
